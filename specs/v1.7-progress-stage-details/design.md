...
图标解析采用无持久化的展示时外部 API 队列：FaviconKit 128px API → Tomba → Brandfetch（配置 `VITE_BRANDFETCH_CLIENT_ID` 时）→ 首字回退。图片的 `load` 事件确认有效尺寸后才显示；`error` 或零尺寸图片推进到下一来源。企业链接变化时来源队列从头开始，不新增公司数据字段或云端迁移。
