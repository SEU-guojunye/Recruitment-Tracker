...
每个节点使用原生 `button` 作为点击与键盘触发面。标记和名称继续沿用现有 Steps 视觉状态；名称旁显示向右的小三角作为可展开提示，展开后旋转向下；`aria-expanded` 指示是否展开，`aria-controls` 指向时间线下方唯一详情面板。
...
图标解析采用无持久化的展示时外部 API 队列：Brandfetch（配置 `VITE_BRANDFETCH_CLIENT_ID` 时）→ Tomba → FaviconKit 128px API → 首字回退。图片的 `load` 事件确认有效尺寸后才显示；`error` 或零尺寸图片推进到下一来源。企业链接变化时来源队列从头开始，不新增公司数据字段或云端迁移。
