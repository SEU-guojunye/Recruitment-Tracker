...
- 从合法的公司招聘链接提取 hostname，依次尝试 Brandfetch 图标 API、Tomba Logo API 和 FaviconKit 128px API；当前外部 API 来源失败后自动切换下一来源。Brandfetch 仅在配置公开的 `VITE_BRANDFETCH_CLIENT_ID` 时启用，未配置时安全跳过。
...
- 节点详情展示环节名称、状态、日期和备注；空日期显示“未填写”，空备注显示“暂无备注”，备注内的 HTTP/HTTPS URL 使用安全的新窗口外链。
...
- Brandfetch、Tomba 和 FaviconKit 外部 API 均加载失败时显示浅主题色圆角矩形和公司名称首字，不显示破损图片。
