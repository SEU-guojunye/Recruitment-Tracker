...
解析器不负责识别申请职位；用户在“我的投递”中手动填写和编辑岗位名称。岗位名称可以为空，投递仍通过投递记录 ID、链接、工作地点、投递时间和备注等字段区分。
...
- 搜索范围包括公司名称、公司招聘链接、岗位名称、投递链接、状态页链接、工作地点、公司备注和投递备注。
...
- 公司备注、岗位名称、投递备注、自定义进度环节、阶段、终态、环节顺序、环节日期和当前环节均可在导出后重新导入并保持一致。
