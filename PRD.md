...
- 最近更新时间。
- 最右侧操作列按“投递、编辑、删除”顺序固定提供三个纯文字控件，不使用边框、背景或胶囊样式，仅通过文字色区分：“投递”使用品牌蓝并进入岗位投递新增操作、预选当前公司；“编辑”使用中性灰并打开当前公司招聘信息编辑操作；“删除”使用错误红并在二次确认后删除当前公司招聘信息及其全部关联岗位投递。

招聘信息列表不展示公司备注或最近进度列。行业类型、招聘批次和优先度均由用户在 Dashboard 维护，点击对应文字时进入编辑操作；优先度同时用于顶部 `P0 公司` 统计。桌面端公司列与招聘链接列的标题和内容保持自然左对齐，其余信息列统一居中，使信息锚点均匀分布；移动端转为卡片布局后，各字段标签和值全部左对齐以保持纵向阅读效率。

桌面端招聘信息表头与数据行必须复用同一套八列轨道，单元格占满各自列宽，禁止根据每行内容独立计算列起点。公司列使用最大权重，确保常见中文公司全称尽量完整显示；优先度和投递岗位数使用紧凑权重，减少短值两侧的无效留白；招聘批次、招聘链接、最近更新和操作使用中等权重。窄桌面端只收紧单元格与操作文字间距，不改变上述相对关系。进入移动端断点后改用两列卡片布局，不产生横向滚动。
...
- 从合法的公司招聘链接提取 hostname，并请求 `https://ico.faviconkit.net/favicon/{domain}?sz=64`。
- 域名必须先校验并进行 URL 编码，不向 FaviconKit 发送完整招聘路径或查询参数。
- 图片加载成功时显示 Favicon；无合法域名、请求失败、返回无效图片或图片解析失败时，才显示公司名称前两个字符作为文字图标。
- Favicon 仅作辅助识别，公司名称始终保留为可读文本；图片使用空 `alt`，避免屏幕阅读器重复播报公司名。
- 图标容器不使用背景色、边框或外环，Favicon 等比铺满原有图标区域，不修改企业原始图标的颜色。
...
- 展开 / 收起控件。
- 公司编辑控件使用与招聘信息页一致的普通文字操作样式，不使用主题色实心按钮；不提供“更多操作”菜单。

公司摘要列顺序固定为“公司、投递链接、投递岗位数、已投递岗位、操作”。桌面端列宽按内容密度分配：公司列与已投递岗位列作为主要内容列，已投递岗位略宽；固定显示短文案的投递链接列使用紧凑宽度，避免文字两侧形成大块空白；投递岗位数使用紧凑但不突兀的数字列，操作列保持固定宽度。五列不得出现某一列显著挤占列表主体空间。岗位投递与招聘信息两个页面的列表表头统一使用 TDesign 占位文字色、13px 字号和 500 字重；数据行统一使用 13px 主文字色，公司名使用 600 字重，计数及普通字段使用 400 字重，链接使用 13px 主题蓝。两页数据行最小高度统一为 64px。
...
投递详情字段在桌面端使用五列布局：投递岗位名称列略宽，其余“岗位链接、岗位工作地点、投递日期、最新更新日期”四列等宽。五列的表头与内容全部左对齐；窄屏端允许按既有响应式规则分行，但不得产生水平滚动。
...
- 招聘信息列表展示公司图标、公司名称、行业类型、招聘批次、优先度、独立的招聘链接列、投递岗位数和最近更新，不展示公司备注或最近进度列；行业类型、招聘批次和优先度均使用与正文列一致的可编辑普通文字，招聘链接使用主题蓝；桌面端公司列最宽、优先度与投递岗位数列紧凑，公司列左对齐、其余列居中，移动端所有字段左对齐。
- FaviconKit 图标加载失败时显示公司名称前两个字符，不显示破损图片。
