...
- 从合法的公司招聘链接提取 hostname，依次尝试 Favicon.im 高清 API 和 FaviconKit 128px API；当前外部 API 来源失败后自动切换下一来源。
...
- Favicon.im 和 FaviconKit 外部 API 均加载失败时显示浅主题色圆角矩形和公司名称首字，不显示破损图片。
